...
This complements the per-channel analysis in run_correlation_stats.py.

Model: correlation_z ~ condition + (1|subject) + (1|channel)
n = 128 observations per condition (4 subjects × 32 channels)

Note: Uses Fisher z-transformed correlations for valid parametric analysis.

Usage:
    python run_correlation_stats_aggregated.py --metric correlation_direct_z --condition1 VIZ --condition2 AUD
    python run_correlation_stats_aggregated.py --metric correlation_max_lagged_z --condition1 VIZ --condition2 AUD
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Aggregated statistical analysis for correlation (one model per band)'
    )
    parser.add_argument('--metric', type=str, required=True,
                       choices=['correlation_direct', 'correlation_direct_z',
                               'correlation_max_lagged', 'correlation_max_lagged_z'],
                       help='Metric to analyze (use _z version for Fisher z-transformed)')
    ...
    print("="*60)
    print("AGGREGATED MIXED-EFFECTS ANALYSIS (CORRELATION)")
    print("="*60)
    ...
    # Apply Fisher z-transformation if needed
    if '_z' not in args.metric:
        print("\nApplying Fisher z-transformation...")
        base_metric = args.metric
        z_metric = f"{base_metric}_z"
        
        if base_metric in df.columns:
            df[z_metric] = df[base_metric].apply(fisher_z_transform)
            metric_to_use = z_metric
            print(f"  Transformed {base_metric} -> {z_metric}")
        else:
            print(f"  Warning: {base_metric} not found, using {args.metric}")
            metric_to_use = args.metric
    else:
        metric_to_use = args.metric
        
        # If _z column doesn't exist, create it
        if metric_to_use not in df.columns:
            base_metric = metric_to_use.replace('_z', '')
            if base_metric in df.columns:
                print(f"\nCreating {metric_to_use} from {base_metric}...")
                df[metric_to_use] = df[base_metric].apply(fisher_z_transform)
            else:
                raise ValueError(f"Neither {metric_to_use} nor {base_metric} found in data")
    
    # Run aggregated model for each band
    print("\nRunning aggregated mixed-effects models...")
    print(f"Model: {metric_to_use} ~ C(condition) + (1|subject) + (1|channel)")
    print()
    ...
    for band in bands:
        print(f"  {band}...", end=' ')
        result = run_aggregated_mixed_model(df, metric_to_use, band, 
    ...
    print()
    print("      The per-channel statistics in run_correlation_stats.py show")
