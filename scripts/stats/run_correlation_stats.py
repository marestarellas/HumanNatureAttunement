...
def run_models_all_channels(df, metric, condition1, condition2):
    ...
    # Report mixed vs OLS fallback split (OLS ignores subject-level random intercept,
    # so its p-values do NOT correct for repeated-subject structure).
    if 'model_type' in results_df.columns:
        n_mixed = int((results_df['model_type'] == 'mixed').sum())
        n_ols = int((results_df['model_type'] == 'ols').sum())
        n_total = n_mixed + n_ols
        if n_ols > 0:
            print(f"\nWARNING: {n_ols}/{n_total} ({100.0 * n_ols / n_total:.1f}%) "
                  f"channel-band fits fell back to OLS (mixed-effects model failed).")
            print("         OLS does not account for subject-level structure; "
                  "results for those rows are anti-conservative.")
            print("         Inspect results_df['model_type'] to filter if needed.")
        else:
            print(f"\nAll {n_mixed} channel-band fits used mixed-effects model.")
...
def plot_topomap_results(results_df, metric, condition1, condition2, output_dir):
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(15, 5.6 * nrows))
    if nrows == 1:
        axes = axes.reshape(1, -1)
    axes = axes.flatten()

    # Pre-compute per-band data + per-band vmax (asymmetric range across bands).
    band_data = {}
    for band in available_bands:
        ...
        band_data[band] = (np.array(values), np.array(sig_mask))

    last_im = None
    for idx, band in enumerate(available_bands):
        ax = axes[idx]
        values, sig_mask = band_data[band]
        if len(values) == 0:
            ax.axis('off')
            continue

        # Per-band symmetric colour scale: avoids one band dominating the others.
        band_vmax = max(float(np.max(np.abs(values))), 1e-6)

        lowcut, highcut = freq_ranges.get(band, (0, 0))

        # MNE info per band (uses just enough channels for this band's row count).
        ...
        im, _ = mne.viz.plot_topomap(
            ...
            vlim=(-band_vmax, band_vmax), contours=0, sensors=False,
        )
        ...
        if np.any(sig_mask):
            ...
            ax.scatter(sig_positions[:, 0], sig_positions[:, 1],
                       s=55, c='white', marker='o', edgecolors='black',
                       linewidths=1.7, zorder=10)

        # Per-band colorbar (small) under each panel.
        cbar = plt.colorbar(im, ax=ax, orientation='horizontal',
                            fraction=0.05, pad=0.06, shrink=0.85)
        cbar.ax.tick_params(labelsize=8)
        cbar.set_label(f"Fisher z (band scale: ±{band_vmax:.3f})",
                       fontsize=8, color='#444', labelpad=2)

        n_sig = int(np.sum(sig_mask))
        ax.set_title(f'{band.upper()} ({lowcut}-{highcut} Hz)\n'
                     f'{n_sig}/{len(sig_mask)} sig.',
    ...
    # Direction reminder (no global colorbar — each panel has its own).
    fig.text(0.99, 0.97,
             f"+ → {condition2}",
...
def main():
    ...
    if args.figures_dir is None:
        figures_dir = project_dir / 'figures' / 'audio_eeg_correlation'
    else:
    ...
    # Output dirs: stats CSVs in results/, plots in figures/
    ...
    # Generate plots (write to figures_dir, not stats output_dir)
    print("\nGenerating plots...")
    plot_topomap_results(results_df, args.metric, args.condition1, args.condition2, figures_dir)
    plot_all_bands_summary(results_df, args.metric, args.condition1, args.condition2, figures_dir)

    # Plot lag distribution if using lagged metric (function reads input from same dir;
    # leave it pointing at results_dir which holds audio_eeg_correlation_results.csv).
