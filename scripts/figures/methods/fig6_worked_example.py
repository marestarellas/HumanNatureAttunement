...
# -*- coding: utf-8 -*-
"""
Methods Figure 6 - Worked example on real pilot data.

Pulls the audio swell envelope (``env_swell_0p2``) and respiration time
series for one (subject, condition) cell of the HNA pilot dataset, runs
each of the four coupling-family canonical estimators on the cleaned
signals, and lays out the results so the methods report can show the
toolbox running on actual measurements (rather than on synthetic
demonstrators only).
...
Layout
------
  Top strip:        time-series preview (audio envelope + cleaned respiration)
  Bottom 2 x 2:     A. Linear  -> windowed xcorr peak |r| over time + lag profile
                    B. Oscillatory -> PLV polar histogram at the swell band
                    C. Information -> joint scatter + Pearson r + MI
                    D. Complexity  -> windowed scaling-exponent traces + r
...
def _panel_complexity(ax, env, resp, fs):
    ...
    ax.text(0.04, 0.04,
            ...
            bbox=dict(boxstyle="round,pad=0.30", fc="#f5f5f5",
                      ec="#cccccc", lw=0.5))
    ax.set_title("D.  Complexity   (windowed scaling-exponent traces)",
                 fontsize=11.5, fontweight="bold", loc="left")
    ax.grid(True, alpha=0.30)
...
def make_figure(env, resp, t, label: str, out_path: Path):
    ...
    fig.text(0.07, 0.965,
             ...
             color="#1F2A37")
    fig.text(0.07, 0.948,
             "Same four-panel structure as Methods Fig 1, applied to one (subject, condition) "
             "cell of the HNA pilot dataset.",
             fontsize=9.5, color="#5A6470", ha="left", va="bottom",
             style="italic")
