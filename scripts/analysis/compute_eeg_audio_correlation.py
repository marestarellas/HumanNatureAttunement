...
Usage:
    python compute_eeg_audio_correlation.py --subjects 2 3 5 --conditions VIZ AUD MULTI
    python compute_eeg_audio_correlation.py --subjects 2 --conditions VIZ
"""
...
def extract_audio_envelope(audio_signal, fs):
    """Amplitude envelope via the analytic signal (Hilbert)."""
    return _hilbert_envelope(audio_signal)


def bandpass_filter_envelope(envelope, lowcut, highcut, fs, order=4):
    """Bandpass filter the audio envelope. Thin wrapper around dsp.bandpass."""
    return _bandpass(envelope, fs=fs, lo=lowcut, hi=highcut, order=order)


def downsample_envelope(envelope, original_fs, target_fs):
    """Downsample envelope to target_fs. Thin wrapper around dsp.resample_to."""
    return _resample_to(envelope, fs_from=original_fs, fs_to=target_fs)


def bandpass_filter_eeg(eeg_signal, lowcut, highcut, fs, order=4):
    """Bandpass filter EEG to a specific band. Thin wrapper around dsp.bandpass."""
    return _bandpass(eeg_signal, fs=fs, lo=lowcut, hi=highcut, order=order)
...
def load_eeg_data(subject_id, condition, data_dir):
    ...
    exclude_cols = ['annotation', 'condition_names', 'time_s', 'audio_sample', 'audio_time_s',
                    ...
                    'env_broad', 'env_swell_0p2', 'env_swell_0p3', 'env_swell_0p1', 'env_splash_1_5']
