...
narrowband around the audio swell-envelope dominant frequency (bandwidth
0.12\,Hz, centred on the Welch-PSD peak of \texttt{env\_swell\_0p2} within
$[0.05, 0.5]$\,Hz); and Kraskov-style mutual information on the analytic
envelopes. Respiration is the source signal for one set of analyses
(paired with each audio envelope); HRV uses two complementary derivations
of the cardiac signal --- a beat-to-beat \emph{instantaneous heart-rate}
trace cubic-interpolated to 4\,Hz from R-peak midpoints (used for the
oscillatory metrics PLV / wPLI / coherence at the swell band), and the
classical \emph{windowed HRV features} (RMSSD, MeanNN, SDNN at 30\,s
windows with 90\,\% overlap, then linearly resampled to 4\,Hz; used for
the slow-trend metrics cross-correlation and MI). Mixing the two
prevents narrowband-Hilbert phase analyses from being applied to
30-s-windowed traces whose effective bandwidth (0--0.017\,Hz) sits well
below the swell band. EEG is band-filtered to match the audio band as
the third pairing. Implementation lives in the open-source
...
\caption{\textbf{HRV--audio coupling: nature vs.\ rest, n=5 subjects.}
Same layout and statistical apparatus as Figure~\ref{fig:nvr_resp}. The
four panels mix two complementary cardiac derivations
(see~\S\ref{sec:methods}, \emph{Cardiac signal --- two derivations}):
\textbf{xcorr peak $|r|$} uses the 30\,s windowed mean-RR-interval
trace (HRV\_MeanNN) interpolated to 4\,Hz; \textbf{PLV / wPLI /
band-avg coherence} use the \emph{instantaneous heart-rate} signal at
4\,Hz, built from R-peak indices via 1/RR cubic interpolation. Audio:
\texttt{env\_swell\_0p2} envelope resampled to the corresponding 4\,Hz
grid by linear interpolation. The slow-trend xcorr panel shows higher
nature-vs-rest values ($\Delta\!=\!+0.017$, paired Wilcoxon
$p=0.625$, LMM on per-window peaks $p=0.037$, $*$). PLV / wPLI / band-
averaged coherence on the instantaneous-HR signal show small or
negative $\Delta$ (PLV: ns; wPLI: ns; coherence \emph{lower} under
nature, LMM $p=0.0034$). However, the surrogate test in
Figure~\ref{fig:surrogate_hrv} shows the absolute values of all three
oscillatory metrics sit at the phase-shuffle null floor, so the
nature-vs-rest contrast on those panels should be interpreted as
\emph{descriptive only} --- it differentiates two cardiac states that
are both at chance against the audio. Only the slow-trend xcorr
panel passes a chance-level reading at this sample size.}
\label{fig:nvr_hrv}
\end{figure}
...
respiration; Figure~\ref{fig:fig5_hrv}, instantaneous HR), the audio-only
condition shows the strongest respiration--audio coupling on phase
metrics, peaking $\sim$2$\times$ the value seen during \textsc{viz} or
\textsc{multi}. The omnibus Friedman test does not reach $p\!<\!0.05$
at n=5 ($p_{\mathrm{wPLI}}=0.165$), but pairwise Wilcoxon comparisons
show \textsc{viz}--\textsc{aud} as the dominant trend. For HR--audio
coupling (measured on the instantaneous-HR signal at 4\,Hz, see
\S\ref{sec:methods}), band-averaged coherence shows a Friedman effect
($p=0.022$, $*$) with \textsc{aud} highest of the three conditions;
PLV and wPLI are not significant. Crucially, all three
HR--audio oscillatory values sit at or below their phase-shuffle
surrogate null (Figure~\ref{fig:surrogate_hrv}), so any rank-order
within this section should be read as a descriptive comparison
between cardiac states that are both at chance against the audio
envelope, not as evidence for audio-driven HR phase coupling at this
sample size.
...
\caption{\textbf{HR--audio oscillatory coupling across the three
multisensory conditions, n=5 subjects.} Phase-locking value (PLV),
weighted phase-lag index (wPLI) and band-averaged coherence
(0.04--0.5\,Hz Welch) computed on the \emph{instantaneous heart-rate}
signal at 4\,Hz (built from R-peak indices via 1/RR cubic
interpolation; see \S\ref{sec:methods}, \emph{Cardiac signal --- two
derivations}). Because the same instantaneous-HR trace is used for
all three metrics, the figure does \emph{not} resolve a separate row
per HRV-feature trace (HRV\_MeanNN, HRV\_RMSSD, HRV\_SDNN); those
windowed-feature traces have effective bandwidth ${\sim}0.017$\,Hz
and cannot support narrowband-Hilbert phase analyses at the
${\sim}0.1$\,Hz swell band. Same statistical apparatus as
Figure~\ref{fig:fig5_resp} (Friedman omnibus + three pairwise
Wilcoxon post-hoc, same significance code). Band-averaged coherence
shows a Friedman effect ($p=0.022$, $*$) with \textsc{aud} highest of
the three conditions and $\sim$-trends on the \textsc{viz}/\textsc{aud}
and \textsc{aud}/\textsc{multi} pairs; PLV and wPLI are not
significant across conditions ($p=0.819$ each). \textbf{Caveat:} the
phase-shuffle surrogate test in Figure~\ref{fig:surrogate_hrv} shows
that the absolute coherence / PLV / wPLI values plotted here all sit
at or below the per-subject phase-shuffle null floor; the
condition rank-orders within this figure should therefore be read
as descriptive comparisons \emph{between cardiac states that are
both at chance against the audio envelope}, not as evidence for
audio-driven HR coupling.}
\label{fig:fig5_hrv}
\end{figure}

\paragraph{Surrogate test: HR--audio oscillatory coupling is at chance.}
A phase-shuffle surrogate test (n=200 surrogates per subject and
condition; the audio envelope is randomised in the Fourier domain to
preserve its spectrum, so the null retains all spectral characteristics
of the real audio) shows that observed coherence, PLV and wPLI between
the instantaneous-HR signal and \texttt{env\_swell\_0p2} sit \emph{at or
below} the surrogate null mean across every condition
(Figure~\ref{fig:surrogate_hrv}). Across 75 (subject $\times$ condition
$\times$ metric) tests only 2 reach $p_{\text{one-sided}}<0.05$ ---
fewer than the 3.75 expected by chance under a true null. Mean
$z$-scores are between $-0.7$ and $+0.4$ across the matrix; the
within-nature ``\textsc{aud} wins on coherence'' rank-order in
Figure~\ref{fig:fig5_hrv} and the ``rest > nature'' coherence
asymmetry in Figure~\ref{fig:nvr_hrv} are therefore \emph{not above
chance} --- they should be interpreted as descriptive within-condition
orderings only, not as evidence for perception-driven HR-audio
coupling. The slow-trend xcorr panel of Figure~\ref{fig:nvr_hrv}
(LMM $p=0.037$) is the only HR-audio finding that survives a
straightforward chance-level comparison; even there, an n=5 LMM
\emph{p}-value should be treated as preliminary.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{Fig_surrogate_hrv_audio_oscillatory.png}
\caption{\textbf{Phase-shuffle surrogate test for HR--audio
oscillatory coupling, n=5 subjects, n=200 surrogates per cell.}
For each (subject, condition) the audio swell envelope
(\texttt{env\_swell\_0p2}) is phase-randomised 200 times and each
metric (band-averaged coherence, PLV, wPLI) is re-evaluated on the
shuffled-audio surrogate, giving a per-cell null distribution. \emph{Top
row}: observed value (filled circle, coloured by condition) with the
per-cell null 95\% interval shown as a thin grey bar. \emph{Bottom
row}: $z = (\text{observed} - \text{null mean}) / \text{null std}$
per (subject, condition); dashed line at $z=1.96$ marks the
one-sided $p<.05$ threshold. Across the three metrics, observed values
sit broadly within or below the null intervals, $z$-scores cluster
near zero with no systematic positive deviation under the
\textsc{aud}/\textsc{viz}/\textsc{multi} contrast, and only 2/75
tests reach $p_{\text{one-sided}}<0.05$ (fewer than the 3.75 expected
by chance). The conclusion is that none of the HR--audio
oscillatory-metric values reported elsewhere in this section rise
above the spectrum-preserving surrogate null at this sample size.}
\label{fig:surrogate_hrv}
\end{figure}
...
Sweeping all five coupling metrics against all 12 audio-band envelopes
gives a per-modality fingerprint of where in the audio's frequency
hierarchy the coupling lives. For respiration
(Figure~\ref{fig:F_resp}), phase-based metrics (PLV, wPLI) lock most
strongly to the slow swell envelopes and to the band that overlaps the
breathing rate (0.15--0.40\,Hz HRV-HF band). For the cardiac side, the
two metric families correspond to the two complementary cardiac
derivations described in \S\ref{sec:methods}: cross-correlation and
mutual information run on the windowed HRV-feature trace
(Figure~\ref{fig:F_hrv_slow}, here HRV\_MeanNN), while PLV, wPLI and
band-avg coherence run on the instantaneous-HR signal at 4\,Hz
(Figure~\ref{fig:F_hr_osc}; the latter does not depend on which HRV
feature is under test, so a single figure suffices).

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{F_HRV_slow_trend_HRV_MeanNN.png}
\caption{\textbf{HRV (mean NN)--audio slow-trend coupling fingerprint,
n=5 subjects.} Group-mean coupling per (audio-band envelope
$\times$ multisensory condition) for the two metrics that operate on
the windowed HRV\_MeanNN trace at 4\,Hz: cross-correlation peak
$|r|$ and Kraskov-style mutual information. Rows = the 12 band-
organised audio envelope columns (broadband, two slow swells, two
HRV-band envelopes, splash band, six EEG bands); columns = the three
conditions (\textsc{viz}, \textsc{aud}, \textsc{multi}). Each panel
has its own colour scale (printed under the colourbar) so within-
metric variation across bands is visible. xcorr peaks on the slow
swells (values up to 0.43 on \texttt{env\_swell\_0p1} during
\textsc{aud}); mutual information dominates on the slow swells too
(values $\geq 0.8$, upward-biased by autocorrelation but consistent
across conditions) and is essentially zero on the EEG bands ---
biologically expected, because the HRV trace cannot share
information with audio amplitude modulations at, e.g., 8--13\,Hz
that have no counterpart in cardiac dynamics.}
\label{fig:F_hrv_slow}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{F_HR_oscillatory.png}
\caption{\textbf{Heart-rate--audio oscillatory coupling fingerprint,
n=5 subjects.} Group-mean coupling per (audio-band envelope $\times$
multisensory condition) for the three metrics that require true
beat-to-beat bandwidth: band-averaged Welch coherence
(0.01--0.5\,Hz), PLV and wPLI. All three are computed on the
\emph{instantaneous-HR signal} at 4\,Hz (1/RR cubic-interpolated;
see \S\ref{sec:methods}, \emph{Cardiac signal --- two derivations}).
Same row / column convention as Figure~\ref{fig:F_hrv_slow}.
Coherence is broadly elevated under \textsc{aud} across most envelope
bands; PLV and wPLI show a diffuse pattern across audio bands. The
phase-shuffle surrogate test in Figure~\ref{fig:surrogate_hrv} (run on
the swell envelope) shows that the corresponding observed values for
this metric trio sit at or below the per-subject null floor at n=5,
so the heatmap should be read as a descriptive map of cell magnitudes
rather than as evidence that any cell rises above chance. The
spatial pattern itself is now interpretable (the previous, pre-fix
heatmap was dominated by interpolation artefacts in a 30-s-windowed
HRV trace at frequencies the trace did not carry); the question of
whether any cell is above chance requires either more subjects or
more recording time per condition.}
\label{fig:F_hr_osc}
\end{figure}
...
Same layout as Figure~\ref{fig:F_hrv_slow}, with cleaned respiration
...
\subsection{EEG--audio band-matched coupling}

For each EEG band we band-pass-filter the audio amplitude envelope and
the EEG to the same band (delta 0.5--4, theta 4--8, alpha 8--13,
low-beta 13--20, high-beta 20--30, gamma$_1$ 30--50\,Hz), then compute
three coupling metrics on the band-matched pair:
\textbf{(a) Pearson correlation $r$} (the existing band-matched
metric, sample-by-sample), \textbf{(b) phase-locking value (PLV)}, and
\textbf{(c) weighted phase-lag index (wPLI)}. The latter two are
computed via narrowband-Hilbert phase extraction in the same band
(\texttt{plv\_phase\_sync} / \texttt{wpli\_phase\_sync} with
$f_0 =$ band centre, $\Delta f$ = band width). The three metrics ask
slightly different questions: the correlation captures sign-aligned
co-variation including amplitude; PLV captures phase locking
amplitude-blind; wPLI is amplitude-blind \emph{and} suppresses
volume-conduction-like zero-lag artefacts.

\paragraph{Statistical inference (LMM with crossed random effects).}
Per (band, metric) we fit a linear mixed-effects model
\texttt{value $\sim$ C(condition, ref="VIZ") + (1|subject) +
(1|channel)} with REML, on the n=5 subjects $\times$ 32 channels
$\times$ 3 conditions = 480 observations per band. The omnibus
condition effect is a 2-df Wald F-test on the joint AUD/MULTI dummies;
pairwise contrasts (AUD vs.\ VIZ, MULTI vs.\ VIZ, AUD vs.\ MULTI) are
Wald t-tests on the corresponding contrast vectors. The crossed RE
structure addresses pseudoreplication across channels within subjects;
at n=5 subjects, however, between-subject variance is poorly
estimated, so the LMM $p$-values should be read alongside the
visible per-subject means in the violin overlays. Significance code:
\texttt{*** $p\!<\!.001$, ** $p\!<\!.01$, * $p\!<\!.05$,
$\sim$ $p\!<\!.10$, ns} otherwise.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{Fig15_eeg_audio_correlation_3cond.png}
\caption{\textbf{EEG--audio band-matched correlation across the three
multisensory conditions, n=5 subjects.} Six EEG bands $\times$ 3
conditions; per-subject means across 32 channels are shown as black
dots, gold diamond + line shows the group-mean trajectory across
conditions, paired connectors link within-subject means.
LMM omnibus $p$ inset (lower right of each panel); pairwise
brackets show the three Wald t-test contrasts from the same model. The
strongest contrasts replicate at the LMM level the broadband VIZ--AUD
pattern previously reported: alpha and high-beta show \textsc{aud}
elevation; delta and theta show \textsc{multi} elevation; gamma$_1$
shows \textsc{multi} > VIZ.}
\label{fig:eeg_corr3}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{Fig18_eeg_audio_plv_3cond.png}
\caption{\textbf{EEG--audio band-matched PLV across the three
multisensory conditions, n=5 subjects.} Same layout and statistical
apparatus as Figure~\ref{fig:eeg_corr3}. PLV is computed via
narrowband-Hilbert phase extraction at the band centre frequency
(\texttt{plv\_phase\_sync} with $f_0$ and $\Delta f$ matched to each
band), so the metric isolates phase locking between the EEG band-rate
oscillation and audio amplitude modulations at the same band-rate
(cortical-tracking style). \emph{Bands where PLV agrees with the
correlation pattern}: alpha (\textsc{aud} highest); low-beta
(\textsc{multi} highest); gamma$_1$ (significant condition effect,
direction differs from correlation). \emph{Where PLV diverges from
correlation}: delta and theta (PLV ranks \textsc{multi}/VIZ above
\textsc{aud} where correlation showed weaker effects). The divergence
is informative: it isolates amplitude-independent phase alignment from
the broader sample-by-sample co-variation captured by Pearson r.}
\label{fig:eeg_plv3}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{Fig19_eeg_audio_wpli_3cond.png}
\caption{\textbf{EEG--audio band-matched wPLI across the three
multisensory conditions, n=5 subjects.} Same layout and statistical
apparatus as Figure~\ref{fig:eeg_corr3}. wPLI debiases PLV against
near-zero-lag phase differences (which can be inflated by volume
conduction or narrowband filter ringing) and is therefore a more
conservative phase-coupling readout. The qualitative condition
rank-orders agree with PLV in alpha (\textsc{aud} highest), low-beta
(\textsc{multi} highest) and gamma$_1$, with the omnibus effect
slightly weaker (theta loses significance at LMM $p=0.143$). The
agreement of wPLI with PLV in alpha and low-beta increases confidence
that those band-specific contrasts are not driven by narrowband
filter / volume-conduction artefacts.}
\label{fig:eeg_wpli3}
\end{figure}
...
audio-only stimulation, where attention is undivided. \emph{(ii)} On the
cardiac side, slow-trend coupling (windowed MeanNN--audio
cross-correlation) is mildly elevated during nature relative to rest
(LMM $p=0.037$). All HR--audio oscillatory metrics (PLV, wPLI,
coherence on the instantaneous-HR signal) sit at or below the
phase-shuffle surrogate null at n=5
(Figure~\ref{fig:surrogate_hrv}), so the descriptive condition
rank-orders for those metrics --- including the ``rest > nature''
asymmetry on coherence and the ``\textsc{aud} highest'' Friedman
effect within nature --- should not be read as evidence for
audio-driven cardiac coupling at this sample size. \emph{(iii)}
Linear methods (cross-correlation) and phase methods (PLV, wPLI)
report complementary information for respiration; for the heart, only
the linear / slow-trend channel currently rises above chance, while
phase channels need either more subjects or longer per-condition
recordings. Information-theoretic methods (MI on Hilbert envelopes)
recover yet a third pattern, dominated by slow-swell co-variation. A
useful prior for analysis choices in the larger study.
...
EEG--audio band-matched coupling, now reported with three complementary
metrics (Pearson $r$, PLV, wPLI; Figures~\ref{fig:eeg_corr3},
\ref{fig:eeg_plv3}, \ref{fig:eeg_wpli3}), shows broad band-specific
differences between conditions. The most consistent pattern across the
three metrics is alpha (\textsc{aud} highest, $p<.05$ at the LMM
omnibus) and low-beta (\textsc{multi} highest), interpretable as
auditory-attention engagement of alpha and a multisensory-binding
fingerprint at low-beta respectively. Where PLV/wPLI disagree with
the correlation pattern (delta, theta), the divergence isolates
amplitude-blind phase alignment from broader sample-level
co-variation --- both are real measurements of different things, and
the larger study should report all three.
...
respiration--audio readout; (2) cardiac coupling requires \emph{two}
parallel signals --- the windowed HRV-feature trace for slow-trend
metrics (xcorr, MI) and the instantaneous-HR trace at 4\,Hz for
oscillatory metrics (PLV, wPLI, coherence) --- because applying
narrowband-Hilbert phase analyses to a 30-s-windowed feature trace is
methodologically incorrect (its effective bandwidth ${\sim}0.017$\,Hz
sits below the swell band); (3) every cardiac-side coupling claim
should be paired with a phase-shuffle surrogate test before any
condition contrast is interpreted as ``audio-driven'' --- the n=5
pilot here shows that all HR--audio oscillatory metrics sit at the
chance ceiling, so the larger study needs either more subjects, or
substantially longer per-condition recordings, or both, to clear that
floor; (4) EEG analyses should plan for both the
pseudoreplication-aware LMM (subject $\times$ channel crossed random
effects) and a small-N-honest paired test on subject means.
