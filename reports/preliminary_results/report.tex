...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{spectrum_overlay.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{nature_vs_rest_resp.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{nature_vs_rest_hrv_meannn.png}
...
grid by linear interpolation. xcorr peak $|r|$ is mildly elevated
under nature (paired Wilcoxon $p=0.625$, LMM on per-window peaks
$p=0.037$, $*$). PLV and wPLI on the instantaneous-HR signal do not
separate the two blocks (paired Wilcoxon $p\geq 0.625$, LMM $p\geq
0.20$). Band-averaged coherence is \emph{lower} during nature than
rest (LMM $p=0.0034$, $\beta=-0.018$ per window) --- consistent with a
more variable, less spectrally peaked HR profile under naturalistic
stimulation. The contrast directionality reported here is the
between-block LMM contrast and is interpreted accordingly; absolute
HR-coupling above-chance claims would require longer per-condition
recordings and more subjects, with surrogate testing, in the planned
larger study.}
\label{fig:nvr_hrv}
\end{figure}
...
respiration; Figure~\ref{fig:fig5_hrv}, instantaneous HR), the
audio-only condition shows the strongest respiration--audio coupling
on phase metrics, peaking $\sim$2$\times$ the value seen during
\textsc{viz} or \textsc{multi}. The omnibus Friedman test does not
reach $p\!<\!0.05$ at n=5 ($p_{\mathrm{wPLI}}=0.165$), but pairwise
Wilcoxon comparisons show \textsc{viz}--\textsc{aud} as the dominant
trend. For HR--audio coupling (measured on the instantaneous-HR signal
at 4\,Hz, see \S\ref{sec:methods}), band-averaged coherence shows a
significant Friedman effect ($p=0.022$, $*$) with \textsc{aud}
highest of the three conditions; PLV and wPLI are not significant
across conditions. Both contrasts are within-subject between-condition
differences; the n=5 sample is too small to make absolute coupling
claims with the necessary surrogate testing.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{resp_audio_metrics_grid.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{hrv_audio_metrics_grid.png}
...
shows a significant Friedman effect ($p=0.022$, $*$) with \textsc{aud}
highest of the three conditions and $\sim$-trends on the
\textsc{viz}/\textsc{aud} and \textsc{aud}/\textsc{multi} pairs; PLV
and wPLI are not significant across conditions ($p=0.819$ each).
The contrasts plotted here are between-condition, within-subject
LMM-style differences; the absolute level of each coupling value at
n=5 with these recording lengths is not informative for above-chance
existence claims (which would need surrogate testing and longer
per-condition recordings, deferred to the planned larger study).}
\label{fig:fig5_hrv}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{phase_polar.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{hrv_slow_trend_meannn.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{hr_oscillatory.png}
...
bands (consistent with the band-avg coherence panel of
Figure~\ref{fig:fig5_hrv}). PLV and wPLI show a diffuse pattern
across audio bands. The pre-fix version of this figure was dominated
by interpolation artefacts in a 30-s-windowed HRV trace at
frequencies the trace did not actually carry; the current heatmap
operates on a signal whose bandwidth genuinely covers the swell band
(${\sim}0$--$2$\,Hz Nyquist) and the spatial pattern is therefore
interpretable as a descriptive map of cell magnitudes.}
\label{fig:F_hr_osc}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{resp_multi_envelope_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{time_resolved_resp_audio_plv.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{time_resolved_resp_audio_wpli.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{time_resolved_resp_audio_coh.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{eeg_audio_correlation_3cond.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{eeg_audio_plv_3cond.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{eeg_audio_wpli_3cond.png}
...
than during silence (LMM-windowed $p\!<\!0.05$), and the locking peaks
under audio-only stimulation, where attention is undivided. \emph{(ii)}
On the cardiac side, slow-trend coupling (windowed MeanNN--audio
cross-correlation) is mildly elevated during nature relative to rest
(LMM $p=0.037$); the corresponding LMM contrast on band-averaged
coherence using the instantaneous-HR signal goes the other way (LMM
$p=0.0034$, lower under nature) --- consistent with a more variable,
less spectrally peaked HR profile when the body engages with the
sound. PLV and wPLI on instantaneous HR do not separate the two
blocks. \emph{(iii)} Linear methods (cross-correlation, coherence)
and phase methods (PLV, wPLI) report complementary information ---
the respiration--audio answer is sharper on phase metrics, the
HR--audio answer on the slow-trend / coherence channels --- and
information-theoretic methods (MI on Hilbert envelopes) recover yet a
third pattern, dominated by slow-swell co-variation. A useful prior
for analysis choices in the larger study.
...
sits below the swell band); (3) above-chance ``audio-driven coupling''
claims should be validated by a phase-shuffle surrogate test in the
full study, which at the planned $n\!=\!30^+$ subjects and longer
per-condition recordings will have the dynamic range to detect real
coupling that the n=5 pilot necessarily cannot; (4) EEG analyses
should plan for both the pseudoreplication-aware LMM (subject
$\times$ channel crossed random effects) and a small-N-honest paired
test on subject means.
