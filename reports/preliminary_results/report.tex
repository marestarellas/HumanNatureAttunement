...
$p=0.625$, LMM on per-window peaks $p=0.037$, $*$). PLV and wPLI on
instantaneous HR do not separate the two blocks (paired Wilcoxon
$p\geq 0.625$, LMM $p\geq 0.20$). Band-averaged coherence is
\emph{lower} during nature than rest (LMM $p=0.0034$, $\beta=-0.018$
per window) --- consistent with a more variable, less spectrally
peaked HR profile under nature stimulation.}
\label{fig:nvr_hrv}
\end{figure}
...
\textsc{viz}--\textsc{aud} as the dominant trend. For HR--audio coupling
(measured on the instantaneous-HR signal at 4\,Hz, see
\S\ref{sec:methods}), band-averaged coherence shows a significant Friedman
effect ($p=0.022$, $*$) with \textsc{aud} the highest of the three
conditions; PLV and wPLI on instantaneous HR are not significant across
conditions.
...
shows a significant Friedman effect ($p=0.022$, $*$) with
\textsc{aud} the highest of the three conditions and a $\sim$-trend
on the \textsc{viz}/\textsc{aud} and \textsc{aud}/\textsc{multi}
pairs. PLV and wPLI are not significant across conditions
($p=0.819$ each).}
\label{fig:fig5_hrv}
\end{figure}
...
Figure~\ref{fig:fig5_hrv}). PLV and wPLI show a more diffuse pattern
than the earlier (pre-fix) version of this figure: previously they
appeared to peak in the HRV-LF/HF range, but those values were
dominated by interpolation artefacts in the 30-s-windowed HRV
trace at frequencies the trace did not actually carry. Here they
operate on a signal whose bandwidth genuinely covers the swell band
(${\sim}0$--$2$\,Hz Nyquist) and the spatial pattern reflects real
phase coupling.}
\label{fig:F_hr_osc}
\end{figure}
...
(LMM $p=0.037$), while PLV / wPLI on the instantaneous-HR signal do not
separate the two blocks; band-averaged coherence is in fact \emph{lower}
under nature (LMM $p=0.0034$), consistent with a more variable, less
spectrally peaked HR profile under naturalistic stimulation. \emph{(iii)}
Linear methods (cross-correlation) and phase methods (PLV, wPLI) report
complementary information --- the respiration--audio answer is sharper on
phase metrics, the HR--audio answer on the slow-trend trace --- and
information-theoretic methods (MI on Hilbert envelopes) recover yet a
third pattern (slow-swell dominant). A useful prior for analysis choices
in the larger study.
...
sits below the swell band); (3) EEG analyses should plan for both the
