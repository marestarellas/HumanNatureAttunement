...
\paragraph{Cardiac signal --- two derivations.} The R-peak series can
be turned into a 1-D coupling-ready trace in two complementary ways,
each appropriate for a different family of metrics:
\begin{itemize}
  \item \emph{Instantaneous heart-rate at 4\,Hz}
        (\texttt{ecg.instantaneous\_hr\_signal}).
        $\mathrm{BPM}_i = 60 / RR_i$ placed at the midpoint of each
        $RR$ interval, then cubic-interpolated onto a regular grid.
        Effective bandwidth is limited only by the target sampling
        rate (~2\,Hz Nyquist at 4\,Hz), so respiratory sinus arrhythmia
        (~0.15--0.4\,Hz) and audio-swell rhythms (~0.05--0.2\,Hz)
        survive intact. \textbf{Use this for oscillatory metrics}
        (PLV, wPLI, coherence) against the audio swell.
  \item \emph{Windowed HRV-feature traces} (\texttt{HRV\_MeanNN},
        \texttt{HRV\_RMSSD}, \texttt{HRV\_SDNN}, \ldots) computed on
        sliding 30\,s windows with 90\,\% overlap and resampled to
        4\,Hz (\texttt{ecg.compute\_rolling\_hrv\_features} +
        \texttt{ecg.interpolate\_hrv\_to\_regular\_grid}). The 30\,s
        window length caps the effective bandwidth at
        $1/(2 \cdot 30) \approx 0.017$\,Hz, well below the swell band.
        \textbf{Use these for slow-trend metrics} (cross-correlation,
        MI on the slow envelope) and condition-level state contrasts.
\end{itemize}
The HNA scripts mix the two: \texttt{compute\_hrv\_audio\_coupling.py}
runs PLV / wPLI / coherence once per condition on the
instantaneous-HR signal, and cross-correlation per HRV-feature trace
within the same condition. Applying narrowband-Hilbert phase analyses
to a 30-s-windowed HRV trace would be meaningless because that trace
has essentially zero power in the swell band.
